...
class ItemEventConsumerTest {
    ...
    @Mock
    private StockItemEventProcessor stockItemEventProcessor;
    ...
    void consume_directMode_dispatchesToProcessor() {
        ...
        String message = """
                ...
                """;
        when(stockItemEventProcessor.supports("ITEM_CREATED")).thenReturn(true);

        itemEventConsumer.consume(recordOf(message));

        verify(stockItemEventProcessor).process(message, "evt-1", "ITEM_CREATED");
    ...
    void consume_inboxMode_enqueuesMessage() {
        ...
        when(inboxEnqueueService.enqueue(
                StockItemEventProcessor.CONSUMER_NAME,
                "evt-2",
                "ITEM_CREATED",
                message
        )).thenReturn(true);
        ...
        verify(stockItemEventProcessor, never()).process(message, "evt-2", "ITEM_CREATED");
    }
    ...
    void consume_skipsUnsupportedType() {
        ...
        String message = """
                ...
                """;
        when(stockItemEventProcessor.supports("ITEM_UPDATED")).thenReturn(false);

        itemEventConsumer.consume(recordOf(message));

        verify(stockItemEventProcessor).supports("ITEM_UPDATED");
        verify(stockItemEventProcessor, never()).process(message, "evt-3", "ITEM_UPDATED");
        verifyNoInteractions(inboxEnqueueService);
    }
    ...
    private void configureDirectMode() {
        ...
        routingProperties.getRouting().put(StockItemEventProcessor.CONSUMER_NAME, routing);
    }
}
