...
public class StoreQueryReadController implements StoreQueryReadApi {
    ...
    @Override
    public ApiResponse<Page<StoreQueryListResponse>> getStores(String keyword, StoreQueryStatus status, Pageable pageable) {
        return ApiResponse.success(storeQueryReadService.getStores(keyword, status, pageable));
    }
