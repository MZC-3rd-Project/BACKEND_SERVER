...
class NotificationHistoryReaderTest {
    ...
    @Test
    void findMyNotifications_returnsViewsWithGroupedDeliveries() {
        ...
        CursorResponse<NotificationHistoryView> result =
                notificationHistoryReader.findMyNotifications(10L, null, 2);

        assertThat(result.getItems()).hasSize(2);
        assertThat(result.isHasNext()).isTrue();
        assertThat(result.getNextCursor()).isEqualTo(CursorUtils.encode(200L));
        assertThat(result.getItems().get(0).deliveries()).hasSize(1);
        assertThat(result.getItems().get(0).deliveries().get(0).status()).isEqualTo("DELIVERED");
        assertThat(result.getItems().get(1).deliveries()).hasSize(1);
        assertThat(result.getItems().get(1).deliveries().get(0).status()).isEqualTo("RETRYING");
    }
}
