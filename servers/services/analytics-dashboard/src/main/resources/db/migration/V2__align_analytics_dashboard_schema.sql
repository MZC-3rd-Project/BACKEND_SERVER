...
ALTER TABLE analytics_journey_event
    ADD COLUMN IF NOT EXISTS event_sequence INTEGER;

WITH ranked_journey_events AS (
    SELECT id,
           ROW_NUMBER() OVER (PARTITION BY event_id ORDER BY occurred_at, id) - 1 AS computed_sequence
    FROM analytics_journey_event
)
UPDATE analytics_journey_event e
SET event_sequence = ranked_journey_events.computed_sequence
FROM ranked_journey_events
WHERE e.id = ranked_journey_events.id;

UPDATE analytics_journey_event
